...
def test_stream_metrics(client: _client.Determined) -> None:
    ...
    for metrics in [
        list(trial.training_metrics()),
        list(client.get_trials_training_metrics([trial.id])),
    ]:
    ...
    for val_metrics in [
        list(trial.validation_metrics()),
        list(client.get_trials_validation_metrics([trial.id])),
    ]:
