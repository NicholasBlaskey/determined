...
def metrics_stream_echo(trial_id):
    resp = api.get("127.0.0.1:8080", f"/trials/{trial_id}/metrics/stream", stream=True)

    # Can too large of a message fail this?
    r = []    
    for line in resp.iter_lines(chunk_size=1024 * 1024): 
        r.append(bindings.v1Step.from_json(json.loads(line)))            
    return r
