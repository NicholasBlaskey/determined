...
func TestGetTasksAuthZ(t *testing.T) {
	...
	canAccessExp, expCanAccessTask := createTestTrial(t, api, curUser)
	authZExp.On("CanGetExperiment", mock.Anything, mock.Anything, mock.MatchedBy(
		func(e *model.Experiment) bool {
			return e.ID == canAccessExp.ID
		})).Return(nil).Once()

	cantAccessExp, expCantAccessTask := createTestTrial(t, api, curUser)
	authZExp.On("CanGetExperiment", mock.Anything, mock.Anything, mock.MatchedBy(
		func(e *model.Experiment) bool {
			return e.ID == cantAccessExp.ID
