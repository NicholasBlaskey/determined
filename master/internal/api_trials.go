...
func errTrialNotFound(trialID int) error {
	return status.Errorf(codes.NotFound, "trial %d not found:", trialID)
}

func (a *apiServer) getTrialAndCheckCanDoActions(
	ctx context.Context, trialID int, actionFunc func(model.User, *model.Trial) error,
) (model.User, *model.Trial, error) {
	curUser, _, err := grpcutil.GetUser(ctx, a.m.db, &a.m.config.InternalConfig.ExternalSessions)
	if err != nil {
		return model.User{}, nil, err
	}

	notFound := errTrialNotFound(trialID)
	t, err := a.m.db.TrialByID(trialID)
	...
	if err = actionFunc(*curUser, t); err != nil {
		return model.User{}, nil, status.Error(codes.PermissionDenied, err.Error())
	}
	return *curUser, t, nil
}
...
// TODO refactor -- don't love it being in 3 places now
func (a *apiServer) TrialLogs(
	req *apiv1.TrialLogsRequest, resp apiv1.Determined_TrialLogsServer,
) error {
	var taskID model.TaskID
	if _, t, err := a.getTrialAndCheckCanDoActions(resp.Context(), int(req.TrialId),
		trialauth.AuthZProvider.Get().CanGetTrialLogs); err != nil {
		return err
	} else {
		taskID = t.TaskID
	}
...
func (a *apiServer) legacyTrialLogs(
	...
	fetch := func(r api.BatchRequest) (api.Batch, error) {
		if time.Now().Sub(timeSinceLastAuth) >= recheckAuthPeriod {
			if _, _, err := a.getTrialAndCheckCanDoActions(ctx, int(req.TrialId),
				trialauth.AuthZProvider.Get().CanGetTrialLogs); err != nil {
				return nil, err
			}
...
// TODO awkward right now since check is in 3 places and hard to test.
// Not sure if we can better though.
func (a *apiServer) TrialLogsFields(
	req *apiv1.TrialLogsFieldsRequest, resp apiv1.Determined_TrialLogsFieldsServer,
) error {
	_, trial, err := a.getTrialAndCheckCanDoActions(resp.Context(), int(req.TrialId),
		trialauth.AuthZProvider.Get().CanGetTrialLogs)
	if err != nil {
		return err
	}
	...
	var oldTimeSinceLastAuth time.Time
	...
	go api.NewBatchStreamProcessor(
		...
		func(lr api.BatchRequest) (api.Batch, error) {
			if time.Now().Sub(oldTimeSinceLastAuth) >= recheckAuthPeriod {
				_, _, err := a.getTrialAndCheckCanDoActions(resp.Context(), int(req.TrialId),
					trialauth.AuthZProvider.Get().CanGetTrialLogs)
				if err != nil {
					return nil, err
				}
				oldTimeSinceLastAuth = time.Now()
			}
	...
	var newTimeSinceLastAuth time.Time
	resNew := make(chan api.BatchResult)
	go api.NewBatchStreamProcessor(
		api.BatchRequest{Follow: req.Follow},
		func(lr api.BatchRequest) (api.Batch, error) {
			if time.Now().Sub(newTimeSinceLastAuth) >= recheckAuthPeriod {
				_, _, err := a.getTrialAndCheckCanDoActions(resp.Context(), int(req.TrialId),
					trialauth.AuthZProvider.Get().CanGetTrialLogs)
				if err != nil {
					return nil, err
				}
				newTimeSinceLastAuth = time.Now()
			}
...
func (a *apiServer) GetTrialCheckpoints(
	...
	if _, _, err := a.getTrialAndCheckCanDoActions(ctx, int(req.Id),
		trialauth.AuthZProvider.Get().CanGetTrialsCheckpoints); err != nil {
		return nil, err
	}
...
func (a *apiServer) KillTrial(
	...
	_, t, err := a.getTrialAndCheckCanDoActions(ctx, int(req.Id),
		trialauth.AuthZProvider.Get().CanKillTrial)
	if err != nil {
		return nil, err
	}
...
func (a *apiServer) GetExperimentTrials(
	...
	if _, _, err := a.getExperimentAndCheckCanDoActions(ctx, int(req.ExperimentId),
		false, expauth.AuthZProvider.Get().CanGetExperimentsTrials); err != nil {
		return nil, err
	}
...
func (a *apiServer) GetTrial(ctx context.Context, req *apiv1.GetTrialRequest) (
	*apiv1.GetTrialResponse, error,
	...
	// Don't love this error formatting (also might be cool to delete FromProto and just eat
	// the double query cost...
...
func (a *apiServer) SummarizeTrial(
	...
	notFound := errTrialNotFound(int(req.TrialId))
	...
	t := model.TrialFromProto(resp.Trial)
	if ok, err := trialauth.AuthZProvider.Get().CanGetTrial(*curUser, t); err != nil {
		return nil, err
	} else if !ok {
		return nil, notFound
...
func (a *apiServer) CompareTrials(ctx context.Context,
	...
	for _, trialID := range req.TrialIds {
		notFound := errTrialNotFound(int(trialID))
...
func (a *apiServer) PostTrialProfilerMetricsBatch(
	...
	for _, batch := range req.Batches {
		...
		if !existingTrials[trialID] {
			if _, _, err := a.getTrialAndCheckCanDoActions(ctx, trialID,
				trialauth.AuthZProvider.Get().CanPostTrialsProfilerMetricsBatch); err != nil {
...
// TODO(nick) authZ with allocations.
...
func (a *apiServer) GetCurrentTrialSearcherOperation(
	...
	exp := actor.Addr("experiments", t.ExperimentID)
...
func (a *apiServer) CompleteTrialSearcherValidation(
	...
	_, t, err := a.getTrialAndCheckCanDoActions(ctx, int(req.TrialId),
		trialauth.AuthZProvider.Get().CanCompleteTrialsSearcherValidation)
	if err != nil {
		return nil, err
	}
	...
	if err = a.ask(exp, trialCompleteOperation{
		...
		op:        searcher.ValidateAfterFromProto(reqID, req.CompletedOperation.Op),
	}, nil); err != nil {
		return nil, err
	}
...
func (a *apiServer) ReportTrialSearcherEarlyExit(
	...
	_, t, err := a.getTrialAndCheckCanDoActions(ctx, int(req.TrialId),
		trialauth.AuthZProvider.Get().CanReportTrialsSearcherEarlyExit)
	if err != nil {
		return nil, err
	}
	...
	trial := actor.Addr("experiments", t.ExperimentID, reqID)
...
func (a *apiServer) ReportTrialProgress(
	...
	_, t, err := a.getTrialAndCheckCanDoActions(ctx, int(req.TrialId),
		trialauth.AuthZProvider.Get().CanReportTrialsProgress)
	if err != nil {
		return nil, err
	}
...
func (a *apiServer) ReportTrialTrainingMetrics(
	...
	_, _, err := a.getTrialAndCheckCanDoActions(ctx, int(req.TrainingMetrics.TrialId),
		trialauth.AuthZProvider.Get().CanReportTrialsTrainingMetrics)
	if err != nil {
		return nil, err
	}
...
func (a *apiServer) ReportTrialValidationMetrics(
	...
	_, _, err := a.getTrialAndCheckCanDoActions(ctx, int(req.ValidationMetrics.TrialId),
		trialauth.AuthZProvider.Get().CanReportTrialsValidationMetrics)
	if err != nil {
		return nil, err
	}
...
// TODO hmmm... (we could do what like TrialLogs did but we need to understand why...)
// TODO auth
...
func (a *apiServer) PostTrialRunnerMetadata(
	...
	_, _, err := a.getTrialAndCheckCanDoActions(ctx, int(req.TrialId),
		trialauth.AuthZProvider.Get().CanPostTrialsRunnerMetadata)
	if err != nil {
		return nil, err
	}
