...
func TestParseDockerFlags(t *testing.T) {
	conf, hostConf, networkConf, err := ParseDockerFlags([]string{"--cpu-shares", "1025"})
	require.NoError(t, err)
	require.NotNil(t, conf)
	require.Equal(t, int64(1025), hostConf.CPUShares)
	require.NotNil(t, networkConf)

	conf, hostConf, networkConf, err = ParseDockerFlags([]string{
	...
	_, _, _, err = ParseDockerFlags([]string{"--this-isnt-a-docker-flag", "false"})
	require.Error(t, err)

	_, _, _, err = ParseDockerFlags([]string{})
	require.NoError(t, err)
}
